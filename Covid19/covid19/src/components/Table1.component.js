...
import AllCaseUser from './AllCase.component';
...
export function SimpleTable(props) {
  ...
  const handleSelectAllClick = event => {
    if (event.target.checked) {
      const newSelecteds = AllCase.map(n => n.name);
  ...
  const emptyRows = rowsPerPage - Math.min(rowsPerPage, AllCase.length - page * rowsPerPage);

  return (
   
   
    <div className={classes.root}>
      <AllCaseUser></AllCaseUser>
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell
                        bgcolor={row.todayCases > 0 ? '#ffd600' : '#e0e0e0'}
                          align="right">
                          {row.todayCases}
                      </TableCell>
                      <TableCell 
                        bgcolor={row.deaths > 0 ? 'red' : 'black'}
                      ...
                      <TableCell
                        bgcolor={row.todayDeaths > 0 ? '#d50000' : 'white'}
                        style={{ color: 'black' }}
                        align="right">
                        {row.todayDeaths}
                       </TableCell>
                      <TableCell bgcolor="success.main"  align="right">{row.recovered}</TableCell>
